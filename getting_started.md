...
This project supports using OpenAI hosted APIs for LLM and embeddings. Set the following environment variables locally (for example in `.env` or Azure App Settings):

- `OPENAI_API_KEY` — Your OpenAI API key (required)
- `OPENAI_MODEL` — Chat model to use (default: `gpt-5.1-chat-latest`)
- `EMBEDDING_MODEL` — Embedding model (default: `text-embedding-3-large`)

If you prefer to use Azure OpenAI for embeddings, set `AZURE_OPENAI_ENDPOINT`, `AZURE_OPENAI_KEY`, and `AZURE_EMBED_DEPLOYMENT` accordingly. The code will prefer `OPENAI_API_KEY` if present.
